...
function DarkMatterGlobe() {
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      grd.addColorStop(0, 'rgba(255, 145, 0, 0.12)');
      grd.addColorStop(0.5, 'rgba(255, 80, 0, 0.03)');
      ...
      for (const s of streams) {
        ...
        sg.addColorStop(0,'rgba(255,145,0,0)'); 
        sg.addColorStop(0.5,`rgba(255,160,0,${s.alpha})`);
        sg.addColorStop(1,'rgba(255,145,0,0)');
      ...
      particles.map(p => {
        ...
        ctx.fillStyle = `hsla(${35 + pulse*5}, 100%, ${50 + depth*30}%, ${alpha})`; 
        ctx.fill();

        // Particle Bloom for closer elements
        if (depth > 0.8) {
          ctx.beginPath(); ctx.arc(screenX, screenY, size*2, 0, Math.PI*2);
          ctx.fillStyle = `rgba(255, 180, 0, ${alpha * 0.15})`;
          ctx.fill();
        }
      });
      ...
      cg.addColorStop(0, `rgba(255, 255, 255, ${0.8+breathe*0.2})`);
      cg.addColorStop(0.2, `rgba(255, 180, 0, 0.6)`);
